...
from transformers import AdamW, get_linear_schedule_with_warmup, BertForSequenceClassification, DistilBertForSequenceClassification
from utils import GeneralProcessor, get_labels, get_task_specific_tokens, compute_metrics, convert_examples_to_features
...
from transformers import (WEIGHTS_NAME, 
    BertConfig, BertTokenizerFast as BertTokenizer, 
    DistilBertConfig, DistilBertTokenizerFast as DistilBertTokenizer
)
...
def evaluate(config, model, tokenizer, prefix=""):
    ...
    labels = get_labels(config.data_dir)
    with open(os.path.join(eval_output_dir, config.task_name), "w") as writer:
        for key in range(len(preds)):
            writer.write("%d\t%s\n" % (key, str(labels[preds[key]])))
    return result
...
def main():
    ...
    elif config.model_type not in ['bert','distilbert']:
        raise ValueError(f"model_type '{config.model_type}' is invalid and must be 'bert' or 'distilbert'")
    ...
    for t in get_task_specific_tokens(config.data_dir):
    ...
    if config.model_type == "bert":
        cl_bertconfig, cl_tokenizer, cl_model = BertConfig, BertTokenizer, BertForSequenceClassification
    elif config.model_type == "distilbert":
        cl_bertconfig, cl_tokenizer, cl_model = DistilBertConfig, DistilBertTokenizer, DistilBertForSequenceClassification

    # Make sure only the first process in distributed training will download model & vocab
    do_lower_case = "-uncased" in config.pretrained_model_name
    tokenizer = cl_tokenizer.from_pretrained(
        config.pretrained_model_name, do_lower_case=do_lower_case, additional_special_tokens=additional_special_tokens)
    bertconfig = cl_bertconfig.from_pretrained(
        config.pretrained_model_name, num_labels=num_labels, finetuning_task=config.task_name)
    model = cl_model.from_pretrained(config.pretrained_model_name, config=bertconfig)

    if config.local_rank == 0:
        # Make sure only the first process in distributed training will download model & vocab
        torch.distributed.barrier()

    # Resize vocab based on new tokens. Note that DistilBert
    # throws a `NotImplemented` error, but we can hack it and
    # call the underlaying `_resize_token_embeddings()` func directly
    model.to(config.device)
    if config.model_type == "bert":
        model.resize_token_embeddings(len(tokenizer))
    elif config.model_type == "distilbert":
        model._resize_token_embeddings(len(tokenizer))
        bertconfig.vocab_size = len(tokenizer)
    ...
    if config.train and (config.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        model = cl_model.from_pretrained(config.output_dir)
        tokenizer = cl_tokenizer.from_pretrained(
    ...
    if config.eval and config.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = cl_model.from_pretrained(checkpoint)
            model.to(config.device)
